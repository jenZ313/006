...
Besides, every piece of our code follows the dependency rule. Throughout our project, the outer layers depend on inner layers and don’t have cross layer dependency. For example, if we got an instruction to login to the system from UI. The controller login page servlet will send the information to our backend. And the specific  command that corresponding to the login will receive the call and make some verify based on the given username and password. Then the response will be send back and the UI will show user the result. 
